<?php if ( HMBKP_Schedules::get_instance()->get_schedule( $schedule->get_id() ) ) { ?>
	...
	<div class="hmbkp-schedule-actions row-actions">
		...
		<a class="hmbkp-run" href="<?php echo esc_url( wp_nonce_url( add_query_arg( array(
						'action'            => 'hmbkp_run_schedule',
						'hmbkp_schedule_id' => $schedule->get_id()
					), hmbkp_get_settings_url() ) ), 'hmbkp-run-schedule' ); ?>"><?php _e( 'Run now', 'hmbkp' ); ?></a>
		|
		...
		<a href="<?php echo esc_url( add_query_arg( array(
					...
					'hmbkp_schedule_id' => $schedule->get_id()
				), hmbkp_get_settings_url() ), 'hmbkp-edit-schedule' ); ?>"><?php _e( 'Settings', 'hmbkp' ); ?></a> |
		...
		if ( 'database' !== $schedule->get_type() ) : ?>
			<a href="<?php echo esc_url( add_query_arg( array(
						...
						'hmbkp_schedule_id' => $schedule->get_id()
					), hmbkp_get_settings_url() ) ); ?>"><?php _e( 'Excludes', 'hmbkp' ); ?></a> |
		<?php endif; ?>
		...
		<?php foreach ( HMBKP_Services::get_services( $schedule ) as $service ) :
			...
			<a href="<?php echo esc_url( add_query_arg( array(
						'action'            => 'hmbkp_edit_schedule',
						'hmbkp_panel'       => 'hmbkp_edit_schedule_settings_' . $service->get_slug(),
						'hmbkp_schedule_id' => $schedule->get_id()
					), hmbkp_get_settings_url() ) ); ?>"><?php echo esc_html( $service->name ); ?></a> |

		<?php endforeach; ?>

		<a class="delete-action" href="<?php echo esc_url( wp_nonce_url( add_query_arg( array(
						'action'            => 'hmbkp_request_delete_schedule',
						'hmbkp_schedule_id' => $schedule->get_id()
					), admin_url( 'admin-post.php' ) ), 'hmbkp_delete_schedule', 'hmbkp_delete_schedule_nonce' ) ); ?>"><?php _e( 'Delete', 'hmbkp' ); ?></a>
